...
I chose these specific packages because they will enable data
manipulation (dplyr, tidyr, tidyverse, lubridate), visualization
(ggplot2), and documentation (knitr).
...
I set the directory first in order to import files without having to
include the full file path each time.
...
I decided to combine the 12 separate, monthly files into a single, large
data set covering all of 2022 for easier analysis.
...
Exported a copy of the new data frame in case of system crash (again),
save corruption (again) or if overwritten when modifying (again).
...
- *date:* strip only the date from ‘started_at’ to determine when riders
  initially rented the bikes and for use in further modifications
...
I see that the values in the ‘duration’ column each have the unit ‘secs’
or seconds. I will have to remove this unit in order to perform
calculations.
...
I exported a copy of the data frame with the new columns before
...
I changed the data type of the ‘duration’ column to numeric to remove
the unit ‘secs’ from the column values and perform calculations.
...
After viewing the data in a separate tab, I saw negative values in the
column. I decided to remove these values in order to not affect the
analysis.
...
I removed 4 columns (start_lat, start_lng, end_lat, end_lng) to cut down
the size of the data set. These latitude and longitude columns were
unnecessary as the start station and end station names would be more
useful in identifying trends among the riders.
...
I decided to change the names of these columns to ones that are more
